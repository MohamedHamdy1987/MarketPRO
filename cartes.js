 ...
 * ✅ PATCHED: Pledge, return, delete with audit and safety locks
 */
import {
...
import { addAuditLog } from "../data.js";
...
window.pledgeCratesToCustomer = async function(custId, custName) {
  ...
  const rows = (window._crateCustomerRows||[])
    .filter(r => r.customer_id === custId);

  const totalCrates = rows.reduce((s,r)=>{
    return s + ((r.quantity||0)-(r.returned||0));
  },0);

  if (totalCrates <= 0) {
    toast('لا توجد عبوات', 'warning');
    return;
  }
  ...
  inputModal({
    title:`🔒 رهن العبوات`,
    fields:[
      { id:'price', label:'سعر العبوة', type:'number', required:true }
    ],
    onSubmit: async(vals)=>{

      if(window._crateLock) return;
      window._crateLock = true;

      try{

        const price = Number(vals.price);
        if(price <= 0) throw new Error('سعر غير صحيح');

        const total = totalCrates * price;

        /* 🔥 حفظ السعر داخل كل سجل */
        await Promise.all(rows.map(r=>{
          const remaining = (r.quantity||0)-(r.returned||0);
          if(remaining <= 0) return null;

          return dbUpdate('customer_crates', r.id, {
            pledged_price: price,
            is_pledged: true
          });
        }));
        ...
        await supabase.from('customer_ledger').insert({
          ...
          customer_id:custId,
          debit: total,
          description:`رهن عدايات (${totalCrates}) × ${price}`
        });

        await addAuditLog('pledge_crates',{
          custId,
          total,
          price
        });
...
window.returnCustomerCrates = async function(rowId, currentReturned, custId, custName){
  ...
  inputModal({
    title:'↩️ إرجاع',
    fields:[
      { id:'qty', label:'عدد', type:'number', required:true }
    ],
    onSubmit: async(vals)=>{

      const qty = Number(vals.qty);
      if(qty <= 0) throw new Error('قيمة غير صحيحة');
      ...
      const newReturned = Number(currentReturned) + qty;

      await dbUpdate('customer_crates', rowId, {
        returned: newReturned
      });

      /* 🔥 خصم الرهن */
      if(crate?.is_pledged && crate?.pledged_price){

        const refund = qty * Number(crate.pledged_price);

        await supabase.from('customer_ledger').insert({
          user_id:(await ensureUser()).id,
          customer_id:custId,
          credit: refund,
          description:`استرجاع عدايات (${qty})`
        });
      }

      await addAuditLog('return_crates',{
        rowId,
        qty
      });
...
window.deleteCustomerCrateRow = function(id){

  confirmModal('تأكيد الحذف؟', async()=>{

    if(window._delLock) return;
    window._delLock = true;

    try{
      await dbDelete('customer_crates', id);

      await addAuditLog('delete_crate',{id});
