    ...
    window.runReconciliationCheck = async function() {
        ...
        try {
            ...
            if (result.status === 'CLEAN') {
                output.innerHTML = `<div class="card" style="text-align:center;padding:2rem;"><div style="font-size:3rem;">✅</div><h3>جميع الحسابات متطابقة</h3><div style="color:var(--c-text-muted);">وقت آخر فحص: ${formatDate(result.timestamp)}</div></div>`;
            } else if (result.status === 'ERROR') {
                output.innerHTML = `<div class="card" style="text-align:center;padding:2rem;border-color:var(--c-danger);"><div style="font-size:3rem;">❌</div><h3>فشلت عملية التسوية</h3><div style="color:var(--c-danger);">${result.error}</div></div>`;
            } else {
                let html = `<div class="card"><h3 style="color:var(--c-warning);">⚠️ تم العثور على ${result.discrepancies.length} فروقات</h3></div>`;
                result.discrepancies.forEach(d => {
                    html += `<div class="card" style="border-right:4px solid ${d.severity === 'HIGH' ? 'var(--c-danger)' : 'var(--c-warning)'};"><div style="display:flex;justify-content:space-between;align-items:center;"><h4>${d.type.replace(/_/g, ' ')}</h4><span class="badge ${d.severity === 'HIGH' ? 'badge-red' : 'badge-yellow'}">${d.severity}</span></div><p style="color:var(--c-text-muted);">${d.message}</p><pre style="margin-top:0.8rem;background:var(--c-surface-3);padding:0.5rem;border-radius:8px;">${JSON.stringify(d.details, null, 2)}</pre></div>`;
                });
