...
export async function getAllCustomerCrateSummaries() {
  ...
  return (data || []).map(r => ({
    ...r,
    customer_name: r.customers?.full_name || r.customer_name || 'عميل'
  }));
}
...
export async function getAllSupplierCrateSummaries() {
  ...
  return (data || []).map(r => ({
    ...r,
    supplier_name: r.suppliers?.name || r.supplier_name || 'مورد'
  }));
}
...
export async function atomicTransaction(steps) {
  ...
  for (const step of steps) {
    try {
      ...
      console.error(`[atomicTransaction] فشل في ${step.type} على ${step.table}:`, err.message);
      return false;
    }
  }
