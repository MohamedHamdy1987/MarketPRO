...
async function getUser() {
  ...
  _currentUser = await ensureUser();
  return _currentUser;
}

/* ───────────────── SUBMIT GUARD ───────────────── */
let _isSubmitting = false;   // ← للتعديل 3
...
async function loadCustomers() {
  ...
  try {
    ...
    if (error) {
      console.error(error);
      toast("تعذر تحميل العملاء", "error");
      return [];
    }
...
async function loadSuppliers() {
  ...
  try {
    ...
    if (error) {
      console.error(error);
      toast("تعذر تحميل الموردين", "error");
      return [];
    }
...
async function requirePIN() {
  return new Promise((resolve, reject) => {   // ← reject أصبح مستخدمًا الآن
    inputModal({
      title: "🔐 تأكيد العملية",
      fields: [
      ...
      onSubmit: async (vals) => {
        try {
          ...
          if (!ok) {
            reject(new Error("❌ الرقم السري غير صحيح"));  // ← يمنع الـ promise المعلقة
            return;
          }
          resolve(true);
        } catch (err) {
...
function buildSearchableDropdown({ containerId, items, placeholder, onSelect }) {
  ...
  function render(filter) {
    ...
    const filtered = q ? safeItems.filter(i => (i.name || "").toLowerCase().includes(q)) : safeItems;
    if (!filtered.length) {
    ...
    listEl.querySelectorAll("[data-id]").forEach(el => {
      el.addEventListener("click", () => {
        ...
        const item = safeItems.find(x => String(x.id) === String(id));
        if (!item) return;
  ...
  if (wrap._outsideHandler) document.removeEventListener("click", wrap._outsideHandler);
  wrap._outsideHandler = handleClickOutside;
...
function openCustomModal({ title, contentHtml, onSubmit, submitLabel = "حفظ", submitClass = "btn" }) {
  ...
  document.getElementById("khazna_modal_submit").addEventListener("click", async () => {
    ...
    if (!btn) return;
    if (_isSubmitting) return;      // ← منع double click
    _isSubmitting = true;
    btn.disabled = true;
...
export async function renderKhaznaPage(app) {
  ...
  try {
    const user = await getUser();
    let treasuries = await getTreasuriesForUser(user.company_id);

    if (!Array.isArray(treasuries)) {
      console.error("treasuries is not array", treasuries);
      treasuries = [];
    }
    ...
    app.innerHTML = `
      ...
      <div class="grid-2">
        ${safeTreasuries.length ? safeTreasuries.map(t => {
          ...
          `;
        }).join("") : `<div class="card">لا توجد خزائن</div>`}
      </div>
    `;
  } catch (e) {
    console.error("KHZ PAGE ERROR:", e);
    toast("تعذر تحميل صفحة الخزنة، حاول مرة أخرى", "error");
  }
}
...
window.openTreasuryDetails = async function (treasuryId) {
  ...
  try {
    ...
    if (txError) {
      ...
      toast("خطأ في تحميل الحركات", "error");
      return;
    }
    ...
    if (customerIds.length) {
      const { data: cList, error: cErr } = await supabase.from("customers").select("id, name").in("id", customerIds);
      if (!cErr) (cList || []).forEach(c => { customerMap[c.id] = c.name; });
    }
    if (supplierIds.length) {
      const { data: sList, error: sErr } = await supabase.from("suppliers").select("id, name").in("id", supplierIds);
      if (!sErr) (sList || []).forEach(s => { supplierMap[s.id] = s.name; });
    }
    ...
    app.innerHTML = `
      ...
      <div style="display:flex;gap:12px;flex-wrap:wrap;margin-top:16px;margin-bottom:16px;direction:rtl;">
        <div style="flex:1;min-width:140px;background:#86EFAC;border-radius:10px;padding:14px 16px;border-right:4px solid #22C55E;">
        ...
        <div style="flex:1;min-width:140px;background:#FED7AA;border-radius:10px;padding:14px 16px;border-right:4px solid #F97316;">
        ...
        <div style="flex:1;min-width:140px;background:#F0FDF4;border-radius:10px;padding:14px 16px;border-right:4px solid #22C55E;">
      ...
      <div style="display:flex;gap:14px;direction:rtl;align-items:flex-start;flex-wrap:wrap;">
        ...
        <div style="flex:1;min-width:260px;">
          <div style="background:#DCFCE7;border-radius:10px 10px 0 0;padding:12px 16px;display:flex;align-items:center;justify-content:space-between;">
            <span style="color:#166534;font-weight:700;font-size:15px;">📥 التحصيلات</span>
            <button onclick="khazna_income_for('${treasuryId}')" style="background:#4ADE80;color:#1A472A;border:none;border-radius:6px;padding:6px 12px;font-weight:700;cursor:pointer;font-size:13px;">➕ تحصيل</button>
          </div>
          <div style="background:#F0FDF4;border:1px solid #BBF7D0;border-top:none;border-radius:0 0 10px 10px;padding:12px;min-height:100px;">
            ${incomeRows.length ? incomeRows.map(renderTxRow).join("") : `<div style="text-align:center;color:#aaa;padding:20px;">لا توجد تحصيلات</div>`}
          </div>
        </div>

        <div style="flex:1;min-width:260px;">
          <div style="background:#FEE2E2;border-radius:10px 10px 0 0;padding:12px 16px;display:flex;align-items:center;justify-content:space-between;">
            <span style="color:#991B1B;font-weight:700;font-size:15px;">📤 المصروفات</span>
            <button onclick="khazna_expense_for('${treasuryId}')" style="background:#F87171;color:#fff;border:none;border-radius:6px;padding:6px 12px;font-weight:700;cursor:pointer;font-size:13px;">➖ مصروف</button>
          </div>
          <div style="background:#FEF2F2;border:1px solid #FECACA;border-top:none;border-radius:0 0 10px 10px;padding:12px;min-height:100px;">
            ${expenseRows.length ? expenseRows.map(renderTxRow).join("") : `<div style="text-align:center;color:#aaa;padding:20px;">لا توجد مصروفات</div>`}
          </div>
        </div>
    ...
    console.error("TREASURY DETAILS ERROR:", e);
    toast("تعذر تحميل تفاصيل الخزنة، حاول مرة أخرى", "error");
  }
};
...
window.khazna_income = async function (prefillTreasuryId) {
  try {
    const user = await getUser();
    const treasuries = await getTreasuriesForUser(user.company_id);
    const customers = await loadCustomers();

    if (!Array.isArray(treasuries) || treasuries.length === 0) {
      toast("لا توجد خزائن متاحة", "error");
      return;
    }
    ...
    openCustomModal({
      ...
      onSubmit: async () => {
        ...
        validateRequired(treasury_id, "اختر الخزنة");
        validateAmount(amount);
        if (source_type === "customer") validateRequired(customer_id, "اختر العميل");
        ...
        if (!field) throw new Error("قناة غير صالحة");    // ← التحقق من القناة
        ...
        invalidateCache();
        await addAuditLog("income_added", payload);        // ← تدقيق
        toast("تمت الإضافة ✅");
    ...
    console.error("INCOME MODAL ERROR:", e);
    toast(e.message || "تعذر إضافة التحصيل", "error");
  }
};
  ...
  try {
    ...
    openCustomModal({
      ...
      onSubmit: async () => {
        ...
        validateRequired(treasury_id, "اختر الخزنة");
        validateAmount(amount);
        if (expense_type === "supplier") validateRequired(supplier_id, "اختر المورد");
        ...
        invalidateCache();
        await addAuditLog("expense_added", payload);       // ← تدقيق
        toast("تم السحب ✅");
        document.getElementById("khazna_custom_modal")?.remove();
        openTreasuryDetails(treasury_id);
      }
    ...
    toast(e.message || "تعذر إضافة المصروف", "error");
  }
};
...
window.khazna_transfer = async function () {
  try {
    ...
    if (!Array.isArray(treasuries) || treasuries.length === 0) {
      toast("لا توجد خزائن للتحويل", "error");
      return;
    }
    ...
    openCustomModal({
      ...
      onSubmit: async () => {
        ...
        if (transfer_type === "treasury_to_treasury") {
          ...
          validateRequired(from_id, "اختر الخزنة المصدر");
          validateRequired(to_id, "اختر الخزنة الهدف");
          if (from_id === to_id) throw new Error("لا يمكن التحويل لنفس الخزنة");

          const from = treasuries.find(t => t.id === from_id);
          if (!from) throw new Error("الخزنة المصدر غير موجودة");
          if (channel === "bank" && !isFinanceManager(from)) throw new Error("البنك للمدير فقط");

          const field = getChannelField(channel);
          if (!field) throw new Error("قناة غير صالحة");
          if (Number(from?.[field] || 0) < amount) throw new Error("رصيد غير كافي");
          ...
          if (!field) throw new Error("قناة غير صالحة");
          if (Number(t?.[field] || 0) < amount) throw new Error("رصيد غير كافي في القناة المصدر");
        ...
        await addAuditLog("transfer_completed", { transfer_type, amount });  // ← تدقيق
        toast("تم التحويل ✅");
        document.getElementById("khazna_custom_modal")?.remove();
        if (targetTreasuryId) {
    ...
    console.error("TRANSFER MODAL ERROR:", e);
    toast(e.message || "تعذر إتمام التحويل", "error");
  }
};
