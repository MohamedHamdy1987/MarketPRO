...
import { formatCurrency, formatDate, toast } from '../ui.js';

// fallback for navigation if openCustomer not defined
window.openCustomer = window.openCustomer || function(id) {
  navigate('customers');
};
...
export async function renderDashboard(app) {
  ...
  <div class='kpi-grid' id='kpi-grid'>
    ${[0,1,2,3,4].map(()=>`<div class='skeleton skeleton-card'></div>`).join('')}
  </div>

  <div class='grid-2'>
    <div class='card'>
      <div class='card-header'>
        <span class='card-title'>🚨 العملاء المتأخرين</span>
      </div>
      <div id='overdue-widget'></div>
    </div>
    <div class='card'>
      <div class='card-header'>
  ...
  // Fetch overdue data once and reuse
  const overdueData = await loadOverdueCustomers(user);

  await Promise.allSettled([
    loadKPIs(user, overdueData.count),
    loadChart(user),
    loadAging(user),
...
function severityLabel(days) {
  if (days > 15) return 'خطر شديد';
  if (days > 7) return 'متأخر';
  return 'متأخر';
}

function overdueDisplay(days) {
  if (days > 15) return `<span style="color:#b91c1c;font-weight:700;">🚨 متأخر ${days} يوم (خطر)</span>`;
  if (days > 7) return `<span style="color:#ef4444;">⏳ متأخر ${days} يوم</span>`;
  return `<span style="color:#ef4444;">⏳ متأخر ${days} يوم</span>`;
}

/* ── العملاء المتأخرين (مرة واحدة) ────────────────────── */
async function loadOverdueCustomers(user) {
  try {
    const container = document.getElementById('overdue-widget');
    if (!container) return { list: [], count: 0 };

    // جلب العملاء ذوي الرصيد الموجب
    ...
    if (!balances?.length) {
      container.innerHTML = `<div class="empty-state" style="padding:24px;"><div class="empty-icon">✅</div><div class="empty-title">لا يوجد عملاء متأخرين</div></div>`;
      return { list: [], count: 0 };
    }

    const customerIds = balances.map(b => b.customer_id);

    // جلب آخر تاريخ معاملة لكل عميل
    ...
    // تجميع آخر تاريخ لكل عميل
    const latestDates = {};
    (invoices || []).forEach(inv => {
      const cid = inv.customer_id;
      if (!latestDates[cid] || new Date(inv.date) > new Date(latestDates[cid])) {
        latestDates[cid] = inv.date;
      }
    });
    ...
    for (const bal of balances) {
      const lastDate = latestDates[bal.customer_id];

      if (!lastDate) {
        // لا توجد معاملات => متأخر جداً
        overdueList.push({
          name: bal.customers?.full_name || 'عميل',
      ...
      const days = overdueDays(lastDate);
      if (days > 7 && Number(bal.balance) > 0) {  // عتبة 7 أيام للظهور في القائمة، مع مستويات شدة
        overdueList.push({
          name: bal.customers?.full_name || 'عميل',
          balance: Number(bal.balance),
          days,
    ...
    // ترتيب تنازلي حسب الأيام
    overdueList.sort((a, b) => b.days - a.days);
    const top = overdueList.slice(0, 6);
    const count = overdueList.length; // إجمالي عدد المتأخرين (كل من تجاوز 7 أيام أو بدون تاريخ)
    ...
    container.innerHTML = top.map((c, i) => `
      <div style="margin-bottom:12px; cursor:pointer;"
        ...
        <div style="display:flex;justify-content:space-between;align-items:center;margin-bottom:4px;">
          ...
          <span class="badge badge-red">${formatCurrency(c.balance)}</span>
        </div>
        <div style="font-size:11px;font-weight:600;">
          ${overdueDisplay(c.days)}
        </div>
      </div>
    `).join('');
    ...
    // تنبيه تلقائي
    if (count > 5) {
      toast(`⚠️ لديك ${count} عملاء متأخرين - الوضع خطير!`, 'error');
    } else if (count > 0) {
      toast(`⚠️ لديك ${count} عملاء متأخرين — راجعهم الآن`, 'warning');
    }

    return { list: overdueList, count };

  } catch (e) {
    console.error(e);
    const container = document.getElementById('overdue-widget');
    if (container) container.innerHTML = `<div style='color:var(--c-danger);padding:16px;'>⚠️ خطأ في تحميل بيانات المتأخرين</div>`;
    return { list: [], count: 0 };
  }
}

/* ── KPIs (المؤشرات الرئيسية) ───────────────────────────── */
async function loadKPIs(user, overdueCount) {
  try {
    const [
      { data: invoices },
      { data: customers },
    ...
    const kpis = [
      ...
      { icon: '⚠️', value: (overdueCount || 0) + ' عميل', label: 'العملاء المتأخرين', color: 'var(--c-danger)' },
    ];
...
async function loadAging(user) {
  try {
    ...
    container.innerHTML = balances.map((b, i) => {
      ...
      return `
      <div style='margin-bottom:12px; cursor:pointer;'
           onclick="openCustomer('${b.customer_id}')"
           onmouseover="this.style.background='var(--c-surface-3)'"
           onmouseout="this.style.background=''">
        <div style='display:flex;justify-content:space-between;align-items:center;margin-bottom:4px;'>
          <span style='font-weight:700;font-size:13px;'>
