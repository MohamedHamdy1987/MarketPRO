...
const _state = {
  ...
  searchTimer: null,
  cache: {},               // search cache
  hasFailedFinancial: false, // flag for UI badge
};
...
const _db = {
  ...
  // حفظ سجل الحركة الفاشلة للمراقبة المالية
  async logFailedTransaction({ sale_id, operation, type, amount, old_amount, entity_id, user_id, invoice_id, product_id, error_message }) {
    /*
      جدول failed_financial_transactions يجب أن يكون موجوداً بالهيكل التالي:
      CREATE TABLE failed_financial_transactions (
        ...
        error_message TEXT,
        status TEXT DEFAULT 'pending', -- pending / retried / resolved / failed
        retry_count INT DEFAULT 0,
        created_at TIMESTAMP DEFAULT now(),
        CONSTRAINT unique_failed_tx UNIQUE (sale_id)
      );
    */
    const { error } = await supabase
    ...
    if (error) console.error("فشل حفظ سجل المراقبة:", error);
  },
...
window._salesOpenProduct = async function(productId, invoiceId) {
  ...
  try {
    ...
    _state.sales   = sales;
    _resetCashier();
    // إعادة تعيين علامة الفشل المالي عند فتح منتج جديد (يمكن تحسينها لاحقًا)
    _state.hasFailedFinancial = false;
...
window._salesSubmit = async function() {
  ...
  try {
    ...
    if (_state.editingId) {
      ...
      if (delta !== 0) {
        try {
          ...
          await _db.logFailedTransaction({
            sale_id: updated.id,
            operation: "update",
            type: updated.type,
            amount: delta,
    ...
    // تحديث الـ badge في الواجهة إذا لزم
