...
async function renderClosedInvoice(app, invoice, products){
  ...
  // FIX: commission must be integer before smart rounding
  ...
  /* ── SAVE settlement results (ONLY ONCE, with DB check) ── */
  // Guard against double-save: check database, not in-memory object
...
if(error){
  console.error(error);
  toast('خطأ في قراءة الفاتورة','error');
  return;
}

  if(existingInvoice?.net == null){
    ...
    // update supplier balance (avoid duplicates)
